...
<body>
    ...
    <!-- 파일 업로드 시 실패하는 경우 에러메세지를 표시한다. 해당 데이터는
    영역에 저장될 것이다.-->
    ...
    일반적인 폼값 전송을 위해 enctype을 명시하지 않는 경우
    	reauest내장객체의 getParameter()로 폼값을 받을 수 있다.
    파일 업로드를 위해 enctype을 위와같이 명시하는 경우
    	request내장객체로는 폼값을 받을 수 없다. cos.jar 확장라이브러리
    	에서 제공하는 MultipartReauest객체를 통해 받아야 한다.
     -->
	...
	<form name="fileForm" method="post" enctype="multipart/form-data"
	 ...
	 <!-- 파일명을 그래도 사용하고, 중복되는 경우 자동으로 인덱스를
	 부여하여 처리하는 방식 -->
    <!--  <form name="fileForm" method="post" enctype="multipart/form-data"
        ...
        작성자 : <input type="text" name="name" value="이지훈" /><br />
